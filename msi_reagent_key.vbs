...
elseif (errRET = 0) then                                    ''ARGUMENTS PASSED, CONTINUE SCRIPT
  ...
  intRET = objWSH.run ("cmd.exe /C " & chr(34) & "cscript.exe " & chr(34) & "C:\temp\chkAU.vbs" & chr(34) & " " & _
    ...
    chr(34) & strKEY & "|" & strSVR & chr(34), 0, true)
...
''SUB-ROUTINES
sub CHKAU()																									''CHECK FOR SCRIPT UPDATE , 'ERRRET'=10 , MSI_REAGENT_KEY.VBS , REF #2 , FIXES #8
  ...
  if (objFSO.fileexists("C:\Program Files (x86)\N-Able Technologies\Windows Agent\cache\" & wscript.scriptname)) then
	...
	if objXML.load("https://github.com/CW-Khristos/scripts/raw/dev/version.xml") then
        ...
        objOUT.write vbnewline & now & vbtab & " - MSI_REAGENT_KEY :  " & strVER & " : GitHub : " & objSCR.text & vbnewline
        objLOG.write vbnewline & now & vbtab & " - MSI_REAGENT_KEY :  " & strVER & " : GitHub : " & objSCR.text & vbnewline
				if (cint(objSCR.text) > cint(strVER)) then
					...
					call FILEDL("https://github.com/CW-Khristos/CW_MSI/raw/dev/msi_reagent_key.vbs", wscript.scriptname)
					''RUN LATEST VERSION
					if (wscript.arguments.count > 0) then             ''ARGUMENTS WERE PASSED
						for x = 0 to (wscript.arguments.count - 1)
